...
public class ThirdPersonWallCover : MonoBehaviour {
    ...
    bool                    creepCamReversedButPlayerIsStillHoldingSameDir = false;
    StealthPlayerCamera.eCamMode    creepLastCamMode = StealthPlayerCamera.eCamMode.far;
    float                   creepCamReversedHDir;
    ...
    float GetCreepValue() {
        ...
        switch (inCover) {
            ...
            case 2:
                ...
                {
                    // Check to see whether the player is still holding in the same direction.
                    // (There's no need to handle a dead area because the Input settings already do so.)
