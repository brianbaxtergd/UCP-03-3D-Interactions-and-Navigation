...
public class HighlightWhenPlayerInTrigger : MonoBehaviour
{
    ...
    public float highlightSecondsPerPulse = 1f;
    ...
    private void Start()
    {
        ...
        {
            Debug.LogWarning("HighlightWhenPlayerInTrigger:Start() - No gameObjectToHighlight set in inspector.");
        }
        else
        {
    ...
    private void Update()
    {
        ...
        if (playerInteractable.playerWithinTrigger)
        {
            ...
            mat.SetFloat("_Border", outlineWidth * (0.5f - Mathf.Cos(Mathf.PI * (Time.time - startTime) / highlightSecondsPerPulse) * 0.5f));
        }
        else
        {
            mat.SetFloat("_OutlineFront", 0f);
